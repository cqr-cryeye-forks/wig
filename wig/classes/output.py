...
from wig.classes.json_parser import transform_json
...
class OutputJSON(Output):
    ...
    def add_results(self):
        ...
        # SUBDOMAIN
        site['data'].extend(
            [{'category': 'subdomain', 'hostname': sub.subdomain, 'title': sub.page_title, 'ip': sub.ip} for sub in
             get('Subdomain')])
        ...
        # TOOLS
        site['data'].extend(
            [{'category': 'Tool', 'name': t.tool_name, 'link': t.link, 'used_for': t.software} for t in get('Tool')])

        # VULNERABILITIES
        site['data'].extend(
            [{'category': 'Vulnerability', 'name': v.software, 'version': v.version, 'link': v.link, 'num': v.num_vuln}
             for v in get('Vulnerability')])
    ...
    def write_file(self):
        ...
        self.json_data = transform_json(self.json_data)
...
class OutputPrinter(Output):
    ...
    def print_results(self):
        ...
        if subdomains:
            ...
            output_lines.extend(
                [_result(result.subdomain, result.page_title[:self.max_col_width], result.ip) for result in subdomains])
        ...
        if vulns:
            ...
            output_lines.extend(
                [_result(result.software + ' ' + result.version, result.num_vuln, result.link) for result in vulns])
        ...
        widths = [max(map(len, col)) for col in
                  zip(*[i for i in output_lines if type(i).__name__ in ['Result', 'Status']])]
        ...
        intro = [
            _header('SITE INFO'),
            _caption('IP', 'Title', ''),
            _info(self.data['results'].site_info['ip'][0],
                  self.data['results'].site_info['title'][:(widths[1] + widths[2])])
        ]
        ...
        for row in output_lines:
            if type(row).__name__ == 'Header':
                ...
                p.build_line(
                    '  '.join((val.ljust(width) for val, width in zip(row, [widths[0], widths[1] + widths[2]]))),
                    'normal', False)
